<template>
  <div id="outline" class="main">
    <el-container>
      <manage-header />
      <el-main  v-if="this.$route.path == '/outline'">
          <template>
            <!-- Scroll down to see the bottom-right button. -->
            <el-backtop target=".main">
                <div
                    style="{
                        height: 100%;
                        width: 100%;
                        text-align: center;
                    }"
                    >
                    <img src="https://blog.ixuchao.cn/usr/themes/Plain-master/images/commentsbg.gif" alt="">
                </div>
            </el-backtop>
          </template>
        <section>
          <h2 class="title">网站概要</h2>
          <p>
            目前有
            <em>6</em> 篇文章, 并有 0 条关于你的评论在 8 个分类中.点击下面的链接快速开始:
          </p>
          <ul>
            <li>撰写新文章</li>
            <li>更换外观</li>
            <li>插件管理</li>
            <li>系统设置</li>
          </ul>
        </section>
        <section id="content">
          <div class="c1">
            <p>最近发布的文章</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c3">
            <p>官方最新日志</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c2">
            <p>最近得到的回复</p>
          </div>
          <div class="c5">
            <p>最近得到的回复</p>
          </div>
        </section>
      </el-main>
      <router-view></router-view>
      <manage-footer />
    </el-container>
  </div>
</template>

<script>
export default {
  name: "",
  components: {},
  data() {
    return {};
  }
};
</script>

<style scoped>
.el-container{
    display: flex;
    flex-direction: column;
}
.el-main {
  background-color: #e9eef3;
  color: #333;
  text-align: center;
  padding-right: 0;
}
.title{
    padding-left: 0;
}
.main {
        height: 100vh;
        overflow-x: hidden;
    }
/* main{
    /* min-height: calc(100vh - 70px);
    width: 70%; */
  /*  display: flex;
    flex-direction: column;
    align-items: center;
    width: calc(100vw);
    flex-grow: 1;flex-shrink: 1;background-color: rgba(111, 11, 11, 0.2);overflow-y: auto;
} */

/* #outline{
    display: flex;
    flex-direction: column;
    align-items: center;
    height: 100%;
    width: 100%;
} */

/* #content{
    /* display: flex;
    justify-content: row;
    width: ;
    background: #eee; */
/* width: 100%;
    padding: 0 200px; /* 左右两侧腾出来*/
/* }  */
/* .c1,.c3{
    position: relative;
    float: left;
    height: 30px;
    width: 200px;
    background-color: #aaa; 
}
.c2{
    height: 30px;
    position: relative;
    width: 100%;
    float: left;
}
.c1{
    margin-left: -100%; /* 越过中间 */
/* left: -200px; /* 回来一点自己的宽度 */
/* }
.c3{
    margin-left: -200px; /* 跑到右边 */
/* left: 200px;
}  */

ul {
  padding-left: 10px;
}
li {
  list-style: none;
  display: inline;
  padding-right: 10px;
}
/* footer{
    /* margin-top: -100px; */
/* height: 0px; */
/* flex-grow: 0;flex-shrink: 0;background-color: rgba(81, 11, 11, 0.2);
    width: 100%;
    display: flex;
    flex-direction: column;
    align-items: center; */
/* }  */
</style>
